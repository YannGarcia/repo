...
int32_t main(const int32_t p_argc, const char** p_argv) {
  ...
  udp_echo_server server(std::string("0.0.0.0"), std::string("192.168.1.36"), port);
  server.start();

  // Send a UDP message to the server
  int32_t result;
  udp_echo client(address.c_str(), port);
  result = client.send(std::string("This is a message from the client sidee"));
