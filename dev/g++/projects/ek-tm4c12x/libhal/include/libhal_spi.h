/**
 * @File      libhal_spi.h
 * @brief     Header file for the SPI Hardware Abstract Layer library.
...
extern "C" {
  ...
  int32_t libhal_spi_get_fd(const uint8_t p_channel);
   /**
    * @fn int32_t libhal_spi_data_read_write(const uint8_t p_channel, uint8_t *p_buffer, const uint32_t p_length)
