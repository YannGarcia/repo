...
void signal_callback_handler(const int signum) { // FIXME Use one callback per signal
  std::cout << "tempmon: Caught signal: " << signum << std::endl;
...
int main(const int argc, const char * argv[]) {
  ...
  if (signal (SIGINT, signal_callback_handler) == SIG_ERR) {
    std::cerr <<  "tempmon: Cannot handle SIGINT" << std::endl;
    exit (EXIT_FAILURE);
  }
  if (signal (SIGTERM, signal_callback_handler) == SIG_ERR) {
    std::cerr << "tempmon::Cannot handle SIGTERM" << std::endl;
    exit (EXIT_FAILURE);
  }
  ...
  if (!daemonMode) {
	...
	std::cout << "tempmon: key pressed was: '" << (char)getchar() << "'" << std::endl;
    ...
    if (forkId < 0) { // Fork error
      std::cerr << "tempmon: fork error: " << std::endl;
      exit(-1);
      ...
      std::cout << "tempmon: Parent exiting..." << std::endl;
      exit(EXIT_SUCCESS);
    }
  }
