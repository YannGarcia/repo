...
int block_write(int file,int dev_addr,int eeprom_addr,unsigned char *buf,int lenght){
  ...
  printf("block_write: eeprom_addr: %02x - dev_addr: %02x - lenght=%d\n", eeprom_addr, dev_addr, lenght);
  ...
  printf("block_write: memory address: %02x - %02x\n", buff[1], buff[0]);
...
int block_read(int file,int dev_addr,int eeprom_addr,unsigned char *buf){
  ...
  printf("block_read: eeprom_addr: %02x - dev_addr: %02x\n", eeprom_addr, dev_addr);
  
  made_address(eeprom_addr,buff);
  printf("block_read: memory address: %02x - %02x\n", buff[1], buff[0]);
  ln=0;
  //bar();
...
void erase(int file, int addr,int eeprom_size){
  ...
  if(block_write(file,addr,i,buf,j/*MAX_BLK_SIZE*/) !=0) {
